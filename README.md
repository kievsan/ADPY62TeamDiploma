...
>> Команды для бота настраиваются в bot_menu.cfg или в _**get_bot_menu_config_json()**_ модуля _**config.py**_
...
>> Требуется самостоятельно создать БД, затем прописать в _**get_db_config_json()**_ модуля _**config.py**_ значения ключей 😎 
